...
func sample(
  ...
  let response = try await client.getInterconnect(
    request: GetInterconnectRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/zones/\(zoneId)/interconnects/\(interconnectId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        zoneId: "[placeholder]", interconnectId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
